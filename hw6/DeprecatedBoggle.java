...
public class DeprecatedBoggle {
    ...
    public static List<String> solve(int k, String boardFilePath) {
        ...
        DeprecatedTrie dictTrie, boardTrie;
        dictTrie = new DeprecatedTrie(new In(dictPath));
        boardTrie = new DeprecatedTrie(dictTrie);

        DeprecatedTrie.Node currentNode = boardTrie.getRoot();
        ...
        List<DeprecatedTrie.Node> nodes = DeprecatedTrie.getAllNextNodes(currentNode);

        while (!nodes.isEmpty()) {
            DeprecatedTrie.Node node = nodes.remove(0);
            ...
            List<DeprecatedTrie.Node> nodeList = DeprecatedTrie.getAllNextNodes(node);
            for (DeprecatedTrie.Node nd : nodeList) {
                nodes.add(0, nd);
            }
    ...
    public void simpleTest() {
        ...
        actual = DeprecatedBoggle.solve(7, "exampleBoard.txt").toArray(new String[0]);
        assertArrayEquals(expected, actual);
    }

    @Test
    public void test100x100() {
        DeprecatedBoggle.solve(7, "smallBoard.txt");
    }

    @Test
    public void test50x50() {
        DeprecatedBoggle.solve(7, "smallBoard2.txt");
    }
}
